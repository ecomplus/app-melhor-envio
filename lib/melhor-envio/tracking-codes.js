const logger = require('console-files')
const meClient = require('./client')
const { getAllLabels, updateLabel, deleteLabel } = require('../database')
const getConfig = require(process.cwd() + '/lib/store-api/get-config')
const errorHandling = require(process.cwd() + '/lib/store-api/error-handling')

module.exports = appSdk => {
  logger.log('>> Inciando rastreio de etiquetas')

  const trackingCodes = () => new Promise((resolve, reject) => {
    getAllLabels().then(labels => {
      const checkStatus = async (labels, queue = 0) => {
        console.log(queue)
        let appConfig
        const label = labels[queue]
        const next = () => {
          queue++
          return checkStatus(labels, queue)
        }

        if (!label || !label.store_id) {
          return resolve()
        }

        // evita buscar configuração do app para o mesmo storeId
        let storeId
        if (storeId !== label.store_id) {
          storeId = label.store_id
          appConfig = await getConfig({ appSdk, storeId }, true)
          if (!appConfig.access_token) {
            return next()
          }
        }

        let order
        try {
          order = await appSdk.apiRequest(label.store_id, `/orders/${label.resource_id}.json`).then(({ response }) => response.data)
        } catch (err) {
          errorHandling(err)
          if (err.response && err.response.status === 404) {
            deleteLabel(label.id).catch(logger.error)
          }
          return next()
        }

        const token = appConfig.access_token
        const sandbox = (appConfig.sandbox)

        meClient({
          url: '/shipment/tracking',
          method: 'post',
          data: {
            orders: [label.label_id]
          },
          token,
          sandbox
        }).then(({ data }) => {
          const orderLabel = order.hidden_metafields.find(metafield => metafield.field === 'melhor_envio_label_id' && data.hasOwnProperty(metafield.value))

          if (!orderLabel || !orderLabel.value) {
            return null
          }

          const tracking = data[orderLabel.value]
          const shippingLines = order.shipping_lines.find(shipping => shipping._id)
          const shippingLinesCurrentStatus = (Array.isArray(order.shipping_lines) && order.shipping_lines[0].status)
            ? order.shipping_lines[0].status.current
            : order.fulfillment_status ? order.fulfillment_status.current : ''
          let updateTo

          switch (tracking.status) {
            case 'posted':
              if (shippingLinesCurrentStatus !== 'shipped') {
                updateTo = 'shipped'
              }
              break
            case 'delivered':
              if (shippingLinesCurrentStatus !== 'delivered') {
                updateTo = 'delivered'
              }
              break
            case 'undelivered':
              if (shippingLinesCurrentStatus !== 'returned') {
                updateTo = 'undelivered'
              }
              break
            default:
              break
          }

          const promises = []
          if (updateTo) {
            let promise = appSdk.apiRequest(storeId, `/orders/${label.resource_id}/fulfillments.json`, 'post', {
              shipping_line_id: order.shipping_lines[0]._id,
              date_time: new Date().toISOString(),
              status: updateTo,
              notification_code: tracking.id
            }).then(() => {
              logger.log(`Status do pedido ${order._id} alterado com sucesso para ${updateTo}`)
            })

            promises.push(promise)
          }

          const orderTrackingCodes = shippingLines.tracking_codes && shippingLines.tracking_codes.find(codes => codes.code === tracking.protocol)

          if (!orderTrackingCodes.code || orderTrackingCodes.code !== tracking.protocol) {
            let promise = appSdk.apiRequest(storeId, `/orders/${label.resource_id}/shipping_lines/${shippingLines._id}.json`, 'patch', {
              tracking_codes: [{
                code: tracking.protocol,
                tag: 'app_melhorenvio',
                link: 'https://melhorenvio.ecomplus.biz/tracking_code=' + label.label_id
              }]
            }).then(() => {
              return updateLabel(tracking.status, label.label_id)
            }).then(() => logger.log(`Tracking code atualizado para o pedido ${order._id} / Protocolo: ${tracking.protocol} / ${storeId}`))

            promises.push(promise)
          }

          return Promise.all(promises)
        }).then(next).catch(err => {
          const { response } = err
          if (response && err.isAxiosError && response.status < 500) {
            const payload = {
              storeId: label.store_id,
              labelId: label.label_id,
              status: response.status,
              data: response.data,
              config: response.config
            }

            logger.error('Tracking_codes_err', JSON.stringify(payload, undefined, 4))
          }

          return next()
        })
      }

      return checkStatus(labels)
    })
  })

  const start = () => trackingCodes().finally(() => setTimeout(() => start(), 5 * 60 * 1000))
  start()
}