module.exports = (order, appConfig, merchantData) => {
  ...
  if (order.buyers && Array.isArray(order.buyers) && order.buyers[0] && order.buyers[0].registry_type === 'j') {
    to.company_document = order.buyers[0].doc_number || ''
  } else {
    to.document = order.buyers[0].doc_number || ''
  }
