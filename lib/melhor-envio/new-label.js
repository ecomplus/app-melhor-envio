module.exports = (order, appConfig, merchantData) => {
  ...
  if (appConfig.sender_info) {
    if (appConfig.sender_info.doc_number) {
      from[appConfig.sender_info.doc_number > 11 ? 'company_document' : 'document'] = appConfig.sender_info.doc_number.replace('.', '').replace('-', '')
    }
    if (appConfig.sender_info.doc_number_type) {
      from.economic_activity_code = appConfig.sender_info.doc_number_type.replace('.', '').replace('-', '')
    }
  }

  const shippingLine = (order.shipping_lines && order.shipping_lines[0]) || {}
  if (shippingLine.from && shippingLine.from.street && shippingLine.from.city) {
  ...
  if (shippingLine.app && shippingLine.app.service_code) {
    ...
    label.service = parseInt(serviceCode.replace('ME ', ''))
    let propAgency
    switch (serviceCode) {
      case 'ME 31':
    ...
    if (Object.hasOwnProperty.call(appConfig, propAgency)) {
      label.agency = appConfig[propAgency]
    }
  }
