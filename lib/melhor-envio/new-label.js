module.exports = (order, appConfig, merchantData) => {
  ...
  if (shippingLines.to && shippingLines.to.phone && shippingLines.to.phone.number) {
    to.phone = shippingLines.to.phone.number
  } else if (order.buyers && Array.isArray(order.buyers) && order.buyers[0] && order.buyers[0].phones && order.buyers[0].phones.number) {
    to.phone = order.buyers[0].phones.number
  } else {
    to.phone = ''
  }

  const package = {
    weight: shippingLines.package && shippingLines.package.weight ? shippingLines.package.weight : '',
