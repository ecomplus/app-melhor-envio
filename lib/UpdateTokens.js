...
module.exports = () => {
  ...
  const task = () => {
    let query = `SELECT id, refresh_token, access_token, store_id FROM melhorenvio_app_auth WHERE updated_at < datetime("now", "-24 hours") OR updated_at IS NULL`
        ...
        await me.auth.refreshToken(row.refresh_token)
          ...
          .catch(error => {
            ...
            try {
              ...
              if (parse.statusCode === 401) {
                let sql = `DELETE FROM melhorenvio_app_auth WHERE id = ?`
                db.run(sql, row.id, function (err) {
                  if (err) {
                    return logger.error('SQLERR', err.message)
                  }
                  logger.log(`MELHOR_ENVIO_REMOVE_TOKEN ${row.id} | STORE ${row.store_id}`)
                })
              }
            } catch (e) {
              logger.error('MELHOR_ENVIO_REMOVE_TOKENS_ERR', e.message)
            }
          })
      }
    })
