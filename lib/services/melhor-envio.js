...
const notifications = async (request, response) => {
  ...
  if (order.response) {
    ...
    if (labelCanBeGenerated(data)) {
      ...
      let schema = await generateLabel(data, seller, publicBody.response.data.data).catch(e => errors(e, 'label_schema_parse'))
          ...
          me.shipment.checkout()
            .then(() => {
              ...
              saveLabel(label, storeId, resourceId)
                ...
                .catch(e => errors(e, 'label_db_update'))
            })
            .catch(e => errors(e, 'label_checkout'))
        })
        .catch(e => errors(e, 'label_generate'))
    } else {
      // se não for possível
      // response com 204
      end()
