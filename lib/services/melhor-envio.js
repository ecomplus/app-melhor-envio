    ...
    me.shipment.calculate(schema)
      .then(services => {
        ...
        if (application.hasOwnProperty('data') && application.data.hasOwnProperty('sort')) {
          objResponse.shipping_services.sort(sortBy(application.data.sort))
        }
      ...
      .catch(e => {
        ...
        response.status(erros.statusCode)
        return response.send(erros.body)
      })
  } else {
...
const notifications = async (request, response) => {
  ...
  // end request with 204 status code
  const end = () => {
    response.status(204)
    return response.end()
  }

  let order = await sdk.apiRequest(storeId, 'orders/' + resourceId, 'GET', null, appAuth).catch(e => errors(e, 'ecomplus_api_request'))
  ...
  if (order.response) {
    ...
    if (labelCanBeGenerated(data)) {
      ...
      let schema = await generateLabel(data, seller, publicBody.response.data.data).catch(e => {
        errors(e, 'label_schema_parse')
        response.status(400)
        return response.end()
      })
              ...
              saveLabel(label, storeId, resourceId)
                .then(() => {
                  ...
                  sdk.apiRequest(storeId, 'orders/' + resourceId + '/hidden_metafields.json', 'POST', params, appAuth)
                    .then(() => end())
                    .catch(e => {
                      errors(e, 'update_order_metafields')
                      response.status(400)
                      return response.end()
                    })
                })
                .catch(e => {
                  errors(e, 'label_db_update')
                  let erros = JSON.parse(e.message)
                  response.status(erros.statusCode)
                  return response.send(erros.body)
                })
...
const verifyLabelStatus = async () => {
  ...
  sql.db.all(query, ['pending', 'released', 'delivered', 'undelivered'], async function (erro, row) {
    ...
    row.forEach(async (label, index) => {
      // seto intervalo entre as requisição
...
const updateTokenMelhorEnvio = () => {
  let query = 'SELECT refresh_token, access_token, store_id FROM melhorenvio_app_auth'
  sql.each(query, (err, row) => {
    if (!err) {
      try {
        me.setToken = row.access_token
        me.auth.refreshToken(row.refresh_token)
          .then(resp => {
            if (resp) {
              let data = { access_token: resp.access_token, refresh_token: resp.refresh_token }
              let where = { store_id: row.store_id }
              sql.update(data, where, 'melhorenvio_app_auth').catch(e => logger.log(new Error('Erro with melhor envio refresh token')))
            }
          })
      } catch (error) {
